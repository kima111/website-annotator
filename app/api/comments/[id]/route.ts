...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string }}){
  ...
  const { error } = await supa.from('comments').delete().eq('id', params.id).eq('user_id', user.id);
  if(error) return NextResponse.json({ error: error.message }, { status: 500 });
  await supa.from('activity').insert({ user_id: user.id, type: 'delete_comment', meta: { id: params.id } });
